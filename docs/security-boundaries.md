...
- Gate E.1 defines provider-neutral authenticated external-principal evidence, Gate E.2 defines deny-by-default authorization-decision evidence, Gate E.3A defines provider-neutral rate-admission evidence, Gate E.3B defines provider-neutral external request-idempotency admission evidence, Gate E.3C defines fail-closed fresh-rate/exact-request admission-composition evidence, and Gate E.4A defines bounded Safe Upload Session reservation evidence for the **exact canonical `platform.safe_upload_session` operation only**. E.4B accepts exact immutable document bytes only inside the finalization contract, requires Gate B before its provider seam, and binds the exact Safe Intake policy version into finalization evidence. These foundations register no public route, execute no authorized operation, write no storage, create no job, and enable no network/orchestration behavior.
...
Gate B completion is a security-foundation milestone, not an activation event. **External document upload remains disabled.** Gate D.1-D.6 durable state/recovery foundations and Gate E.1-E.4B external admission/session/finalization foundations exist. E.4B accepts exact session-bound bytes only through the contract library, finalizes them through `decide_safe_intake()`, and creates no HTTP, storage, or job authority. E.4C must still bind accepted immutable source/hash evidence to server-owned source/job identity. Production session/finalization/rate/idempotency providers, edge/anonymous abuse controls, resource scope where applicable, production storage/runtime activation, privacy-safe live API behavior, and production-readiness controls remain separately required.
...
Gate D.6 covers representative job/candidate/storage partial-output and crash-window convergence at the contract layer. E.1-E.4B cover external principal/authorization/rate/idempotency/composition/session-reservation/finalization confusion and fail-closed admission behavior without activating a public route. E.4A specifically rejects an unrelated valid E.3C operation before the session provider seam; E.4B rejects expired or malformed sessions, mutable payloads, unsafe documents, media-policy confusion, policy-version substitution, callback authority mutation, and same-session/different-document conflicts.

## 11. Security stop rules

The following capabilities remain blocked until their protecting controls and negative tests pass:

- **External upload** → Safe Intake Gate B is complete; Gate D.1-D.6 state/recovery foundations and Gate E.1-E.4B external security/session/finalization foundations exist. Upload remains blocked because E.4C Immutable Source / Job Binding is incomplete, no public upload route is active, and provider-backed session/finalization/rate/idempotency/persistence/storage plus edge/anonymous-abuse and privacy controls remain incomplete.
