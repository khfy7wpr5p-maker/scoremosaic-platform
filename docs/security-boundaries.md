...
Gate B completion is a security-foundation milestone, not an activation event. **External document upload remains disabled.** Gate D.1-D.6 durable state/recovery foundations and Gate E.1-E.4 contract/convergence foundations exist. E.4B accepts exact session-bound bytes only through the contract library and finalizes them through `decide_safe_intake()`. E.4C binds that accepted evidence to deterministic server-owned source/job/storage identity without performing a write, and E.4 closure verifies the A→B→C handoff. Production session/finalization/rate/idempotency providers, edge/anonymous abuse controls, resource scope where applicable, provider-backed persistence/object storage, privacy-safe live API behavior, and production-readiness controls remain separately required.
...
E.4B has the analogous stateful-provider obligation. Exact replay must return the original finalization identity, timestamp, document identity, Safe Intake policy version, and bounded evidence; reuse of the same session for a different document must conflict atomically. The contract verifies exact receipt binding and rejects policy-version substitution, but only the future stateful provider can prove that replay evidence came from the original stored finalization record.

E.4C and the E.4 closure add no provider-backed state. E.4C reuses Gate D.3 authority to derive source/job/storage identity, while the closure verifier independently re-derives that E.4C result from exact E.4B evidence. A structurally valid but post-construction-modified E.4C decision is not authoritative merely because its fields have valid shapes.
...
Gate D.6 covers representative job/candidate/storage partial-output and crash-window convergence at the contract layer. E.1-E.4 cover external principal/authorization/rate/idempotency/composition/session-reservation/finalization/source-binding confusion and fail-closed admission behavior without activating a public route. E.4A specifically rejects an unrelated valid E.3C operation before the session provider seam; E.4B rejects expired or malformed sessions, mutable payloads, unsafe documents, media-policy confusion, policy-version substitution, callback authority mutation, and same-session/different-document conflicts; E.4C and closure reject tampered source/job lineage and post-construction/cross-finalization confusion.

## 11. Security stop rules

The following capabilities remain blocked until their protecting controls and negative tests pass:

- **External upload** → Safe Intake Gate B, Gate D.1-D.6 state/recovery foundations, and Gate E.1-E.4 external security/upload-to-source contract/convergence foundations exist. Upload remains blocked because no public upload route is active and provider-backed session/finalization/rate/idempotency/persistence/storage plus edge/anonymous-abuse and privacy controls remain incomplete. The next development direction is a minimum staging vertical slice, not another abstract E.4 micro-gate.
