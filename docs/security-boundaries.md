...
- Current **public** Gateway document upload and execution/orchestration remain disabled.
- Gate E.1 defines provider-neutral authenticated external-principal evidence, Gate E.2 defines deny-by-default authorization-decision evidence, Gate E.3A defines provider-neutral rate-admission evidence, Gate E.3B defines provider-neutral external request-idempotency admission evidence, Gate E.3C defines fail-closed fresh-rate/exact-request admission-composition evidence, and Gate E.4 is complete as the bounded upload-to-source contract/convergence foundation. E.4A reserves exact canonical `platform.safe_upload_session` evidence, E.4B requires exact immutable document bytes to pass Gate B before finalization, E.4C derives deterministic server-owned source/job/storage identity using existing Gate D.3 authority, and the E.4 closure verifier re-derives E.4C from exact E.4B evidence to reject replay/tamper/cross-finalization confusion. Those contracts themselves register no public route or engine execution capability.
- The minimum staging vertical slice adds one **private staging-only** provider-backed path from exact staging E.3C evidence through E.4A/E.4B/E.4C to a create-once immutable source filesystem write. It exposes no external route, no production provider, and no network/orchestration authority.
...
Gate B completion is a security-foundation milestone, not an external activation event. **Public external document upload remains disabled.** Gate D.1-D.6 durable state/recovery foundations and Gate E.1-E.4 contract/convergence foundations exist. The minimum staging slice now demonstrates stateful E.4A reservation, stateful E.4B exact replay/conflict handling, and one source-only private staging write after E.4C verification. Production session/finalization/rate/idempotency providers, edge/anonymous abuse controls, resource scope where applicable, broader provider-backed persistence/object storage, privacy-safe live API behavior, and production-readiness controls remain separately required.

E.4A replay handling has one explicit stateful-provider trust obligation. An exact replay must return the original stored session identity, creation time, expiry, and budgets; it must not refresh TTL or widen budgets. The private minimum staging provider now demonstrates this behavior. A future production provider must preserve the same semantics and operational evidence.

E.4B has the analogous stateful-provider obligation. Exact replay must return the original finalization identity, timestamp, document identity, Safe Intake policy version, and bounded evidence; reuse of the same session for a different document must conflict atomically. The private minimum staging provider demonstrates these exact-replay/conflict semantics; it is not a production provider.

E.4C and the E.4 closure remain authority derivation/verification boundaries. E.4C reuses Gate D.3 authority to derive source/job/storage identity, while the closure verifier independently re-derives that E.4C result from exact E.4B evidence. In the minimum staging slice only, exact source bytes may be materialized **after** that verification at the server-derived key. A structurally valid but post-construction-modified E.4C decision is not authoritative merely because its fields have valid shapes.

The minimum staging provider additionally fails closed on malformed persisted session state, pre-existing symlink state-path components, payload hash/size mismatch, and a different existing file at the immutable source key. Exact replay may observe the existing matching source but must never overwrite it.
...
Gate D.1-D.6 provide the contract/convergence foundation for immutable artifact authority, storage-key derivation, SHA-256 content binding, provenance chaining, idempotency, restart decisions, and partial-output/crash-window recovery checks. E.4C consumes the existing Gate D.3 authority model for the external upload-to-source handoff.

The minimum staging slice is the first provider-backed storage use: it materializes only the exact verified source bytes under the E.4C server-derived immutable key in a private staging filesystem root, with create-once semantics and collision rejection. It does not persist broader job/candidate/provenance state and is not the production object-storage provider.

Production immutable object storage, production durable database/replay adapters, and production provider-backed provenance persistence are **not enabled**.
...
Gate E.1 safe principal evidence excludes raw subject and credential material. Gate E.2 safe authorization evidence exposes only bounded principal/operation/decision information and does not disclose unrelated policy grants. Gate E.3A safe rate evidence excludes backend/provider diagnostics and does not grant execution authority. Gate E.3B safe idempotency evidence excludes the raw client key, request body, slot ID, request digest, subject, credentials, and provider details. Gate E.3C safe admission evidence excludes internal binding/request material and provider diagnostics while explicitly retaining all execution authority flags as disabled. Gate E.4A safe session evidence exposes only bounded session identity/state/expiry/intake budgets. Gate E.4B safe finalization evidence exposes bounded session/finalization identity, exact Safe Intake policy version, format and size/page-or-image evidence, but no raw document bytes or original filename. E.4C safe evidence exposes bounded server-owned source/job/storage identity and cryptographic bindings; the E.4 closure verifier returns no new outward authority. The minimum staging result may report only bounded IDs/hash and whether the exact source write was `written` or `replay`; it exposes no source bytes, filename, provider root, or transport authority. Privacy-safe behavior for future live HTTP errors and request logs remains required before public API activation.
...
- E.4 replay divergence, cross-finalization binding confusion, and post-construction E.4C source substitution;
- corrupted staging session/finalization state;
- staging state-path symlink escape attempts;
- same-session different-document conflicts;
- mismatched source payload and immutable-source collision/overwrite attempts;
- unauthorized review/approval/publication attempts.

Gate D.6 covers representative job/candidate/storage partial-output and crash-window convergence at the contract layer. E.1-E.4 cover external principal/authorization/rate/idempotency/composition/session-reservation/finalization/source-binding confusion and fail-closed admission behavior without activating a public route. E.4A rejects an unrelated valid E.3C operation before the session provider seam; E.4B rejects expired or malformed sessions, mutable payloads, unsafe documents, media-policy confusion, policy-version substitution, callback authority mutation, and same-session/different-document conflicts; E.4C and closure reject tampered source/job lineage and post-construction/cross-finalization confusion. The minimum staging slice adds provider-backed replay/corruption/path-containment/source-collision regressions while keeping public upload and execution closed.

## 11. Security stop rules

The following capabilities remain blocked until their protecting controls and negative tests pass:

- **External upload** → Safe Intake Gate B, Gate D.1-D.6 state/recovery foundations, Gate E.1-E.4 external security/upload-to-source foundations, and the minimum private staging source-ingest slice exist. Public upload remains blocked because no public route is active and production auth/rate/idempotency/session/finalization/persistence/storage plus edge/anonymous-abuse and privacy controls remain incomplete. Further work should extend the real staging job lifecycle rather than create abstract E.4 micro-gates.
- **Live Gateway dispatch** → Gate C internal-dispatch contracts and Gate D state/recovery contract/convergence foundations are present, but operational credentials/replay persistence, durable provider-backed job/provenance state, live receiver/network wiring, process/worker control, and explicit orchestration activation remain blocked.
