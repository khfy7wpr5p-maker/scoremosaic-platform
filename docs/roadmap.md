...
| Minimum private staging source ingest | Implemented on `main` by PR #89 | Exact E.3C admission can drive stateful E.4A reservation, E.4B Safe Intake finalization, E.4C verification, and one create-once immutable staging filesystem source write. Exact replay converges without overwrite; public HTTP, engine dispatch, orchestration, and production providers remain disabled. |
| Controlled staging runtime | In progress: initial lifecycle plus read-only recovery | The exact immutable staging source now converges to authenticated create-once D.1 planned state, D.2 empty idempotency, and D.4 initial provenance per fixed engine run. A restarted provider can authenticate and freshly rederive that exact record before returning read-only D.5 `pre_dispatch_candidate` decisions. Queue, worker, transition writes, automatic recovery execution, dispatch, orchestration, and engine calls remain disabled. |
...
- Controlled staging job lifecycle and planned-state recovery — consumes only the successful minimum slice, re-verifies its immutable source and E.4B/E.4C lineage, persists authenticated create-once D.1 planned state, D.2 empty idempotency, and D.4 initial provenance for each fixed engine run, then permits exact read-only D.5 recovery after provider restart. It creates no execution authority and is documented in [`controlled-staging-job-lifecycle.md`](controlled-staging-job-lifecycle.md).
...
- extend the controlled staging runtime from authenticated read-only planned recovery into a separately reviewed provider-backed transition write before engine dispatch is considered;
