...
| Controlled staging runtime | In progress: initial durable lifecycle/provenance slice | The exact immutable staging source now converges to authenticated create-once D.1 planned state, D.2 empty idempotency, and D.4 initial provenance per fixed engine run. Queue, worker, transitions, recovery execution, dispatch, orchestration, and engine calls remain disabled. |
...
- Controlled staging job lifecycle — consumes only the successful minimum slice, re-verifies its immutable source and E.4B/E.4C lineage, then persists authenticated create-once D.1 planned state, D.2 empty idempotency, and D.4 initial provenance for each fixed engine run. It creates no execution authority and is documented in [`controlled-staging-job-lifecycle.md`](controlled-staging-job-lifecycle.md).
...
- extend the controlled staging runtime from initial planned evidence into separately reviewed provider-backed recovery/transition behavior before engine dispatch is considered;
