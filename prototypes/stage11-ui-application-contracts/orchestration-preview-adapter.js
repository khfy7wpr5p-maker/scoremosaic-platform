(() => {
  ...
  const EXPECTED_COMMIT = 'f5ee0e605e62c41af86255712941305b2a8c7afb';
  ...
  const valid = () => {
    ...
    if (data.schemaVersion !== 'scoremosaic-st-orchestration-local-preview-data-v1') return false;
    ...
    if (data.target?.mainCommit !== EXPECTED_COMMIT) return false;
  ...
  window.ScoreMosaicOrchestrationPreviewAdapter = Object.freeze({
    ...
    read() {
      if (!valid()) {
        return Object.freeze({state: 'rejected', data: null, error: Object.freeze({code: 'ORCHESTRATION_PREVIEW_INVALID', message: 'Local orchestration preview evidence failed closed.'})});
      }
