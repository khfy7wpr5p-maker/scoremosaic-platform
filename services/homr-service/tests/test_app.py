...
class RouteTests(unittest.TestCase):
    ...
    def test_health_is_available_without_engine(self) -> None:
        response = route_request("GET", "/health?ignored=1", self.config)

        self.assertEqual(response.status, 200)
        self.assertEqual(response.payload["status"], "ok")
        self.assertFalse(response.payload["engine"]["installed"])
        self.assertFalse(response.payload["conversionEnabled"])

    def test_ready_is_false_until_homr_is_installed(self) -> None:
        response = route_request("GET", "/ready", self.config)

        self.assertEqual(response.status, 503)
        self.assertEqual(response.payload["reason"], "homr_engine_not_installed")
        self.assertFalse(response.payload["conversionEnabled"])
