...
import scoremosaic_audiveris.candidate_safety as candidate_safety
...
class RuntimeTests(unittest.TestCase):
    ...
    @staticmethod
    def _write_valid_mxl(path: Path, *, root: str = "score-partwise") -> None:
        with zipfile.ZipFile(path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            archive.writestr(
                "META-INF/container.xml",
                '<container><rootfiles><rootfile full-path="score.musicxml"/></rootfiles></container>',
            )
            archive.writestr("score.musicxml", f"<{root}/>")

    def test_transcription_returns_bound_candidate_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            workspace = root / "workspace"
            workspace.mkdir()
            ...
            self.assertEqual(len(handoff.evidence.raw_artifact_sha256), 64)
            self.assertEqual(len(handoff.evidence.accepted_content_sha256), 64)
    ...
    def test_transcription_fails_closed_if_candidate_changes_after_validation(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            real_validate = candidate_safety.validate_mxl_file
            changed = False
            ...
            def validate_then_tamper(path: Path):
                ...
                if not changed:
                    changed = True
                    self._write_valid_mxl(path, root="score-timewise")
                return evidence

            with patch(
                "scoremosaic_audiveris.runtime.validate_mxl_file",
                side_effect=validate_then_tamper,
            ):
                with self.assertRaisesRegex(
                    RuntimeExecutionError,
                    "audiveris_candidate_unsafe:candidate_handoff_evidence_mismatch",
                ):
                    transcribe_file(input_path, output_dir, config, runner=runner)
