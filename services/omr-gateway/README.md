...
The controlled staging job-lifecycle slice starts only from that successful result. It re-verifies the E.4B/E.4C lineage and immutable source, freshly reconstructs the canonical plan/lifecycle/D.3 manifest, and persists authenticated create-once D.1 `planned` revision `0`, D.2 empty idempotency, and D.4 initial provenance evidence for Audiveris, HOMR, and Clarity. After provider restart, the recovery slice authenticates and exactly compares the stored record with fresh derivation before evaluating read-only D.5 `pre_dispatch_candidate` decisions. Exact replay converges without overwrite; malformed state, a wrong integrity key, source substitution, or symlink redirection fails closed. No queue, worker, transition write, automatic recovery execution, dispatch, orchestration, or engine call is enabled.
...
- controlled staging job lifecycle with authenticated create-once initial D.1/D.2/D.4 evidence and read-only planned-state D.5 recovery after immutable-source reverification
