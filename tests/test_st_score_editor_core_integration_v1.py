...
class STScoreEditorCoreIntegrationV1Tests(unittest.TestCase):
    ...
    def test_bridge_is_fail_closed_and_pinned_to_new_core(self) -> None:
        ...
        self.assertIn("const addressMatches = (address, target) =>", BRIDGE)
        self.assertIn("address.kind !== target.kind || address.eventId !== target.eventId", BRIDGE)
        self.assertIn("return target.kind === 'event' || address.noteId === target.noteId", BRIDGE)
        self.assertIn("if (HAS_REAL_INPUT)", BRIDGE)
        self.assertIn("REAL_SCORE_INPUT_INVALID", BRIDGE)
        self.assertIn("REAL_SCORE_TARGET_MISMATCH", BRIDGE)
